...
class Parallax {
  constructor(options = {}) {
    this.options = {
      ...
      mobileDisabled: options.mobileDisabled !== undefined ? options.mobileDisabled : PARALLAX_CONFIG.MOBILE_DISABLED,
  ...
  parseElementConfig(element) {
    if (!Validation.validateDataAttribute(element, DATA_ATTRIBUTES.PARALLAX, 'boolean')) {
    ...
    if (element.hasAttribute(DATA_ATTRIBUTES.PARALLAX_SPEED)) {
      const customSpeed = parseFloat(element.getAttribute(DATA_ATTRIBUTES.PARALLAX_SPEED));
      
      if (Validation.isNumber(customSpeed) && customSpeed >= 0 && customSpeed <= 2) {
        speed = customSpeed;
      } else {
        console.warn(`Invalid parallax speed for element: ${customSpeed}. Using default.`);
      }
    }
  ...
  setupEventListeners() {
    ...
    this.handleResize = Performance.debounce(() => {
      ...
      if (this.shouldDisableOnMobile() && Validation.isMobile()) {
        this.disable();
      } else if (!this.isEnabled && !Validation.isMobile()) {
        this.enable();
      }
    }, 150);
  ...
  shouldDisableOnMobile() {
    return this.options.mobileDisabled;
  }
  ...
  updateElement(item, scrollY, viewportHeight) {
    ...
    if (!this.isElementInViewport(originalPosition, scrollY, viewportHeight)) {
      return;
    }
    ...
    const elementProgress = (scrollY + viewportHeight - elementTop) / 
  ...
  addElement(element, customSpeed = null) {
    ...
    this.updateElement(this.elements[this.elements.length - 1], this.lastScrollY, window.innerHeight);
