...
class NavManager {
    constructor(options = {}) {
        this.options = {
            mobileBreakpoint: options.mobileBreakpoint || BREAKPOINTS.LAPTOP, // ← ИЗМЕНИТЬ с MOBILE на LAPTOP
            closeOnResize:
                options.closeOnResize !== undefined
    ...
    findElements() {
        ...
        // ОСНОВНОЙ ЭЛЕМЕНТ МЕНЮ - ДОБАВИТЬ поиск по data-mobile-menu
        this.menuElement = DOM.find(
            `[${DATA_ATTRIBUTES.MOBILE_MENU}], nav, [role="navigation"]`
        );
    }
    ...
    handleOutsideClick(event) {
        ...
        const isClickInsideMenu =
            this.menuElement && this.menuElement.contains(clickedElement);
        const isClickOnToggle = this.toggleElements.some(element =>
            element.contains(clickedElement)
        );
        const isClickOnClose = this.closeElements.some(element =>
            element.contains(clickedElement)
        );
    ...
    focusFirstMenuItem() {
        ...
        const focusableElements =
            this.menuElement.querySelectorAll(focusableSelectors);
